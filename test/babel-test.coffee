...
assertStrictEqual = (code, expected, options)->
    expected = babel.transform(expected, noopts).code
    actual = babel.transform(code, spopts).code

    assert.strictEqual actual, expected
    assert.strictEqual babel.transform(expected, spopts).code, expected
    return
...
describe 'babel', ->
    ...
    it 'should transform nested spClick + spRepeat + spShow + spModel', ->
        ...
        expected = """(condition ? _.map(locales, function(locale) {return <button  />}.bind(this)) : void 0)"""
        assertStrictEqual code, expected
        ...
        expected = """(condition ? _.map(locales, function(locale) {return <button onClick={ (function(event, domID, originalEvent) { this.changeLanguage(event, lng) }).bind(this) }  />}.bind(this)) : void 0)"""
        assertStrictEqual code, expected
        ...
        expected = """_.map(locales, function(locale, lng) {return (<button  type="button" className="btn btn-default">
            { i18n.t('name', {lng: locale}) }
            { (condition ? _.map(locales, function(locale) {return <button   onClick={ (function(event, domID, originalEvent) { this.changeLanguage(event, lng) }).bind(this) }/>}.bind(this)) : void 0) }
        </button>)}.bind(this))"""
        assertStrictEqual code, expected

        code = """<button spRepeat="(locale, lng) in locales" type="button" className="btn btn-default">
            { i18n.t('name', {lng: locale}) }
            <button spRepeat="locale in locales" spClick={ this.changeLanguage(event, lng) } spShow={condition}/>
        </button>"""
        expected = """_.map(locales, function(locale, lng) {return (<button  type="button" className="btn btn-default">
            { i18n.t('name', {lng: locale}) }
    ...
    it 'should transform spRpeat extended', ->
        ...
        """ 
        expected = """
        <InputText input={{ 'select': _.map(states, function (state, index) {
                return <option value={state.abbrev} key={index}>
                    {state.abbrev}
                </option>;
            }.bind(this))
        }} className="col-md-6" spModel={[user, "state"]} label={i18n.t("label.state")} />;
